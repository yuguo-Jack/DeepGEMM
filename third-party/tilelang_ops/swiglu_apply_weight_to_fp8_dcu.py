...
try:
    import megamoe as _runtime
except Exception:
    try:
        import deep_gemm as _runtime
    except Exception:
        _runtime = None


@tilelang.jit
def _swiglu_apply_weight_to_fp8_dcu_tl(
    half_hidden: int,
    ...
    def main(
        ...
        out_sf: T.Tensor[get_sf_shape(num_tokens, half_hidden, num_per_channels, False, use_col_major_scales), out_sf_dtype],  # type: ignore
        ...
        for i, j in T.Parallel(blk_n, blk_h):
            ...
            y_frag[i, j // num_per_channels, j % num_per_channels] = (
                gate_frag[i, j] / (1 + T.exp(-gate_frag[i, j])) * up_frag[i, j] * topk_weight[i]
            )
        ...
        for i, j in T.Parallel(blk_n, blk_h // num_per_channels):
            ...
            sf, sf_inv = get_sf_and_inv(clamped_amax, round_scale, False)
    ...
    @T.prim_func
    def _swiglu_apply_weight_to_fp8_dcu(
        x: T.Tensor[(num_tokens, half_hidden * 2), in_dtype],  # type: ignore
        topk_weights: T.Tensor[num_tokens, w_dtype],  # type: ignore
        avail_tokens: T.Tensor[1, num_dtype],  # type: ignore
        out: T.Tensor[(num_tokens, half_hidden), out_dtype],  # type: ignore
    ...
    return _swiglu_apply_weight_to_fp8_dcu


def swiglu_apply_weight_to_fp8_dcu(
    x: torch.Tensor,
    ...
    assert fmt == "e4m3"
    if ue8m0_scale:
        raise ValueError("DCU TileLang SwigLU FP8 path requires ue8m0_scale=False")
    if num_sms is None:
        num_sms = _runtime.get_num_sms() if _runtime is not None and hasattr(_runtime, "get_num_sms") else 24
    ...
    y_sf = torch.empty(
        get_sf_shape(num_tokens, hidden_size // 2, num_per_channels, False, use_col_major_scales),
        device=x.device,
        dtype=torch.float32,
    )
    ...
    if num_tokens > 0:
        _swiglu_apply_weight_to_fp8_dcu_tl.pass_configs = {
            tilelang.PassConfigKey.TL_DISABLE_WARP_SPECIALIZED: True,
            tilelang.PassConfigKey.TL_DISABLE_TMA_LOWER: True,
            tilelang.PassConfigKey.TL_ENABLE_FAST_MATH: fast_math,
        }
        kernel = _swiglu_apply_weight_to_fp8_dcu_tl(
            hidden_size // 2,
            num_per_channels,
