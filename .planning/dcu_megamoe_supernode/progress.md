...
## 2026-07-14 - Flash LL Kernel Optimization Restart

- User requested a new correctness-preserving LL kernel optimization round because framework-integrated Flash decode currently shows no improvement.
- Required workflows were restored: `planning-with-files`, `hygon-hip-kernel-optimizer`, `dcu-rag-kb`, and `dcu-supernode-workflow`.
- Active plan remains `.planning/dcu_megamoe_supernode`; session catchup reported no unsynced context.
- Local `git status --short` initially showed only `.planning/dcu_megamoe_supernode/task_plan.md` modified; its diff is mojibake/checkmark cleanup and is preserved.
- Defined the fast validation matrix: Flash EP8 then EP16, tokens `8,32,64,128,256,512`, `ll-masked` baseline, per-bucket CUDA graph timing, existing precision/nonfinite/stat checks, and representative `single-local-rank` skew.
- 151.1 was checked immediately before this request: `sglang_megamoe` was up, all 16 HCUs reported `VRAM=0%`, `HCU=0%`, and no KFD PIDs. Recheck again before profiling.
- No production source or remote artifact has been changed in this optimization round yet.
- Ran initial deep DCU KB retrieval for gfx938 FP8 MoE low-latency GEMM/combine and peer-signal synchronization. Results were Hygon-specific but mostly adjacent DeepEP/FlashAttention patterns rather than a direct MegaMoE patch; exact original sources will be inspected before selecting a method.
- Read the optimizer catalog, method registry, metrics guide, and ISA signature registry. No method is selected yet; current profiling will determine the compute/memory/latency axis budget before source edits.
- Followed the DCU KB DeepEP hit into original `internode_ll.cu`; recorded its chunk-level compute-ready combine protocol as a comparison target. Current MegaMoE already has a related split-tail protocol, so no code change is justified without direct hotspot evidence.
- Verified 151.1 mapped-source hashes for the LL hot path and test harness; all matched local. Confirmed current source-tree `.so` artifacts, DTK `hipprof`/`dccobjdump`, and a clean 16-card state before baseline collection.
- Audited the integrated harness and historical Flash runs to freeze a fair, fast baseline contract: explicit LL/`ll-masked`, RPC transport, seed 1234, per-bucket graph capture, tokens `8,32,64,128,256,512`, same-run graph-vs-graph timing, and skew correctness. Confirmed that `--skip-bench` removes only eager timing and leaves graph validation intact.
- Collected the fresh Flash EP8 baseline on 151.1. All correctness checks passed; LL graph medians were `0.5050/0.6549/0.6946/0.7757/1.0664/1.8218 ms`, paired `ll-masked` medians were `0.6081/0.7382/0.7908/0.9108/1.1587/2.1204 ms`, and LL won all six buckets. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_baseline_20260714_114627`.
- Verified the installed DTK 26.04.1 `hipprof` interface before profiling. This build uses `-h` rather than GNU `--help`; `--stats --hip-trace --follow-fork --devices 0`, `-o`, PMC modes, kernel filters, and code-object analysis are present. An initial attempt to truncate `--help` through remote `sed` failed because the tool rejected `--help`/the piped command; no GPU work or files were changed, and the retry with the tool's native output succeeded.
- Profiled the Flash EP8 LL graph at token32 without the baseline path. The profile itself completed and produced `hipprof.db`; only a non-critical final `find | sort` wrapper command failed from a trailing CR. Preliminary device-0 stats put K1 masked group GEMM and K3 combine/reduce above K3 local GEMM and K2; absolute instrumented durations are not used as benchmark latency.
- Incorporated the user's framework decode TPOT table for input lengths 4096/16384 and concurrency 1..64. Results alternate between gains and regressions, with the largest reported gains at 4096/concurrency32 (`+10.60%`) and 16384/concurrency16 (`+11.48%`), but regressions reach `-8.11%` at 4096/concurrency2. These points will be used as the later integration validation surface after isolated branch selection.
- User requested a README wording correction: the example currently labeled as one captured LL graph is actually default per-bucket capture because it omits `--cuda-graph-single-capture`. The documentation fix is now in scope.
- Corrected both LL and normal README graph examples: they now describe the default per-bucket graph cache and state explicitly that `--cuda-graph-single-capture` is required for one max-capacity graph replayed across all prefixes. Commands and runtime behavior were not changed.
- Completed the matching Flash EP8 token512 graph-only profile. K1 and K3 combine appear mostly fixed across token32/token512 under instrumentation, while K3 local GEMM grows and K2 remains small. Selected the first single-variable branch: retain BM32/K3 geometry and compile out only K1's empty second 16-row repeat in the existing Flash tiny specialization.
- A source lookup initially used a stale guessed `src/kernel` path and two exact-file filters returned no match. No files were touched. A broader `rg --files -uuu megamoe` lookup resolved the production header to `megamoe/dcu_megamoe_opt/K1_fused/k1_v3_pack5_groupgemm_impl.cuh`; subsequent inspection uses that path.
- Paused the K1 tiny-repeat edit after exact source review exposed a skew-safety question: specialization selection uses host `valid_rows_per_expert`, while actual per-expert work uses device `gemm_m`. The caller must prove the former is a hard bound before any second-repeat removal; otherwise the branch is rejected before compilation.
- Completed the K3 read-only audit. Its top candidate is a narrowly scoped split-reducer A/B that batches topk6 vector loads two at a time before `vmcnt(0)`, without changing address order, FP32 accumulation order, chunk signals, barriers, or fences. This becomes the fallback if K1 cannot pass the bound proof.
- Traced `valid_rows_per_expert` to `ll_expected_rows_per_expert` and separately confirmed exact physical capacity uses the all-routes worst case. Rejected compile-time second-repeat deletion before editing. Reframed the first branch as a device-count-driven, block-uniform partial-tile guard that preserves all repeats for skew/full tiles.
- Implemented the local K1 partial-tile ablation in `K1_fused/k1_v3_pack5_groupgemm_impl.cuh`. For the existing `kMaskTinyStore` specialization, `active_m_repeats` now comes from device-side `cur_tokens` and the current tile; empty repeats skip scale/A loads, shuffles, MMAC, and stores. Full skew tiles still execute every repeat and the existing per-row partial-store guard remains.
- Local `git diff --check` passed. The branch changes one production header plus the requested README wording and planning records; no other production source changed.
- Synced only the modified K1 implementation header to 151.1's mapped repository. Local and remote SHA256 both equal `6e8fc572c32a70f259294fb889c383fee90a46fd6a609d264ba16d4f54ada444` before rebuild.
- Remote source-contract tests passed (`13 passed`), the build script removed both stale K1 objects, rebuilt/synced fresh runtime extensions, verified all artifacts, and imported every source-tree module. Build run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/k1_partial_repeat_build_20260714_115702`.
- The K1 partial-repeat A/B passed precision but failed performance decisively: token `8/32/64` graph replay regressed to `2.0103/2.3879/2.8151 ms`; token128+ stayed near baseline. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k1_partial_repeat_20260714_120010`.
- Reverted the rejected K1 production-header change locally with an exact reverse patch. The README correction and planning records remain.
- Verified the local K1 header has no source diff and its restored SHA256 is `df81aa39a7af893f676b50678fc6a7bce5e3d6b51bd9559b237113381879d006`; synced it to 151.1 and confirmed the same remote hash. A restore rebuild is required before K3 testing so the rejected binary cannot leak into later measurements.
- Completed the K1 restore rebuild and artifact/import verification. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/k1_restore_build_20260714_120216`. The remote runtime is back on the pre-ablation K1 before any K3 work.
- Inspected the exact K3 reducer/helper source and fixed the paired-load implementation contract: one inline-assembly block must contain load0, load1, and `vmcnt(0)`; results are accumulated in original topk order. This avoids relying on compiler scheduling around a detached no-wait helper.
- Saved the restored baseline K3 V3 extension as `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/k3_pair_artifacts/k3_v3_baseline.so` (SHA256 `41abd49f6f96f430894c0e8aeaeba2c623932d3bbb2fe314574d8976e332676c`) before the candidate build.
- Tooling note/errors: `dccobjdump --help` prints valid options but returns exit 4, so `set -o pipefail` aborted the first combined analyzer script. `hipprof --codeobj-analyze` is interactive for a fat `.so`; without selecting ELF index 1 it writes `error input` and returns 4. Several follow-up stdin scripts also exposed PowerShell-to-SSH CR insertion on a final quoted argument. No source or runtime artifact changed; resource collection is being retried with direct SSH commands/non-interactive selection.
- `dccobjdump --list-elf` and `--extract-elf=all` succeeded and extracted the gfx938 device ELF from the baseline K3 `.so`. This DTK build's `--show-resource-usage`/metadata invocation printed only the file format, and `hipprof --codeobj-analyze` continued to demand interactive selection even for the extracted ELF; piped index 1 was rejected. Treat generic code-object analysis as degraded for now. Preserve both baseline binaries and use `dccobjdump --show-sass`/ELF metadata plus runtime profile for candidate evidence rather than pretending an empty resource report is conclusive.
- Implemented the local K3 split-reducer paired-load A/B. A new inline helper emits exactly two `global_load_dwordx4` plus one `s_waitcnt vmcnt(0)` in a single volatile assembly block. The topk6 path processes three fixed pairs and accumulates packed0 then packed1; other topk values keep the old helper loop. `git diff --check` passed; candidate header SHA256 is `462a86bd1dff32196d48e44c53a775248621deb57478ab08a18a79d579cada68`.
- Synced the K3 header, passed source-contract tests (`13 passed`), rebuilt both stale K3 extension objects, verified/synced runtime artifacts, and imported all modules. Build run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/k3_pair_build_20260714_120959`.
- First EP8 K3-pair screen passed all precision checks and showed sub-2% gains for tokens32..256, near-flat token8, and a noisy token512 median regression. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_pair_20260714_121405`. Proceeding to alternating 20-replay binary A/B before selection.
- Saved candidate K3 V3 `.so` SHA256 `b11e77cd712fc353fcab5ec4796f7aa62e5a1080166518f2fb0ff98647c424a6`, swapped in the saved baseline binary for a 20-replay matrix, and restored the candidate binary afterward. Baseline run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_ab_baseline20_20260714_121617`.
- Candidate 20-replay run reproduced stable `~1.4-1.8%` gains at tokens64/128/256, smaller token32 gain, and flat token8/512 with all graph precision checks passing. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_ab_candidate20_20260714_121720`. It is evidence-positive but still sub-threshold; proceed to ISA/resource inspection and an isolated batch3 test.
- ISA tooling recovery: `dccobjdump --show-sass` requires an output directory and generated ~24.8 MB baseline/candidate ISA files after device-ELF extraction. The container lacks `rg` and DTK lacks `llvm-objdump` at the guessed path, so remote `grep`/`diff` are used as fallbacks. One nested-regex command had a shell quoting error; it did not mutate files.
- The first ISA diff did not show the expected load/wait change in `k3_v3_fused_ext.so`, exposing a likely wrong-artifact A/B. Paused batch3 and all performance claims while tracing whether the reducer is actually compiled into `k3_fused_ext.so`.
- Traced the staged split call end to end and confirmed it does use `k3_v3_fused_ext.so`; the wrong-artifact hypothesis is rejected. A Windows `rg` call with a literal `*.cu` path also produced a harmless glob error before the direct file inspection. Next step is a persisted full SASS diff rather than filtered streaming output.
- Persisted and inspected the full `dccobjdump --show-sass` text diff (`isa.diff`, 66 lines). Its visible body only changed branch offsets, but subsequent `llvm-readelf -sW` showed the exact combine/reduce symbol grew by 836 bytes for Flash and 864 for Pro. Corrected the provisional no-op interpretation: code did materialize; the text dump omits/misrepresents the inserted internal body. Continue with symbol-bounded ELF inspection rather than treating the A/B as noise.
- Compared AMDGPU metadata via `llvm-readelf -n`: batch2 keeps Flash combine/reduce at 39 VGPR, 100 SGPR, 4 SGPR spills, zero VGPR spills, wave64, and no private segment. Function-filtered/recompile/disassembleAll `dccobjdump` modes still omit the internal body, so compiler save-temps is now the preferred sequence-verification route.
- Replaced batch2 locally with an isolated batch3 variant: topk6 is grouped as `(0,1,2)` and `(3,4,5)`, with three vector loads and one wait per group and unchanged slot accumulation order. No other source path changed; `git diff --check` passed and batch3 header SHA256 is `24ac669dab1df9cb458f8bdd2745f5a23a62bdd5871e05efcf648d69257c432f`.
- Synced/rebuilt batch3, passed source-contract tests (`13 passed`), and verified artifacts/imports. Build run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/k3_batch3_build_20260714_122747`. Saved batch3 `.so` SHA256 `6245cabbfb2d5215ea26e7ce6b4261b2ca44da7ede274a33b4ae7fa7bee18d91`.
- First batch3 20-replay EP8 matrix passed precision and improved token64/128/256 by about `2.42%/2.02%/2.08%`, token32 by `1.44%`, with token8/512 flat. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_batch3_20_20260714_123211`. Proceeding to adjacent binary-swap retakes.
- Recovery note before the adjacent retake: an initial local `rg` command used Bash brace-list syntax in PowerShell and failed at parse time; it was immediately replaced with explicit file arguments. No source, remote artifact, or GPU state changed.
- Reconstructed the retake invocation from the retained run log and current harness parser: Flash EP8, explicit LL/`ll-masked`, tokens/capacity 512, per-bucket `8,32,64,128,256,512`, correctness-iters 0, graph warmup 3, and 20 replays. A read-only remote `find -printf` probe rendered literal `n` separators through nested PowerShell quoting; direct argument-array reads confirmed the files and avoided the display issue.
- Completed the adjacent batch3 baseline retake with the saved original K3 V3 binary. All six graph correctness checks passed; original medians were `0.5049/0.6564/0.6957/0.7782/1.0549/1.8373 ms`. The script restored batch3 SHA256 `6245cabb...bee18d91` and `hy-smi --showpids` was clean. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_batch3_ab2_baseline20_20260714_123724`.
- The baseline test body returned status 0, but a trailing CR injected at the final stdin-script line changed `exit 0` into an invalid numeric argument and made only the outer SSH wrapper report failure. The result JSON, binary restoration, and clean-card check all completed before that wrapper error; later scripts end with a harmless command instead of forwarding status on the final line.
- Completed the immediately adjacent batch3 retake. All precision checks passed and runtime SHA remained the saved batch3 artifact. Candidate medians were `0.5021/0.6468/0.6786/0.7649/1.0415/1.8135 ms`; gains versus the adjacent original were `0.56%/1.46%/2.46%/1.71%/1.27%/1.29%`. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_batch3_ab2_candidate20_20260714_123836`.
- Completed a stricter 100-replay original/candidate A/B at tokens `64/128/256` with graph warmup 10. Original medians were `0.69518/0.77708/1.05436 ms`; batch3 medians were `0.67832/0.76318/1.04358 ms`, for `2.43%/1.79%/1.02%` gains. Both runs passed graph precision checks, both post-run card checks were clean, and the runtime remains batch3. Runs: `ep8_k3_batch3_ab3_baseline100_mid_20260714_124029` and `ep8_k3_batch3_ab3_candidate100_mid_20260714_124135` under the active optimization directory.
- EP8 `single-local-rank` token128 passed both eager and graph correctness against `ll-masked` with `max_abs=0.000244141`; post-run cards were clean. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_batch3_skew_t128_20260714_124312`.
- EP16 saved-original full 20-replay matrix passed all six buckets with medians `0.3671/0.4194/0.4971/0.6114/1.0058/1.9485 ms`; batch3 was restored and cards were clean. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep16_k3_batch3_ab_baseline20_20260714_124427`.
- EP16 batch3 then passed tokens `8/32/64/128/256` correctness and reached `0.3628/0.4089/0.4804/0.6018/0.9880 ms`, but token512 triggered `HSA_STATUS_ERROR_MEMORY_APERTURE_VIOLATION` and the spawned ranks aborted. The harness returned status 1 and `hy-smi --showpids` was clean afterward. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep16_k3_batch3_ab_candidate20_20260714_124559`. This is a hard safety blocker; do not retain or rerun unchanged.
- Queried the DCU KB for gfx938 multi-load inline assembly and wait semantics. It returned Hygon instruction/reference examples but no direct early-clobber rule. Local source inspection supplied stronger project-specific evidence: existing multi-instruction `ds_read` helpers in the same K3 header use `=&v` for every output that is written before all inputs are consumed.
- Fixed the batch3 helper's three output constraints from `=v` to `=&v`. This prevents an earlier `global_load_dwordx4` destination from overlapping a later address input. Local `git diff --check` passed; fixed header SHA256 is `7cadcc4041ece3ea9cd7c4aa2da3dd5aa3c333a48e03d8c1286f68d10eccbb5e`.
- Synced the fixed header to 151.1 with matching SHA, passed the remote source-contract suite (`13 passed`), and completed a full artifact/import rebuild. Fixed K3 V3 runtime SHA256 is `e32d9b94784d584ad7fdd0aaf699938742d4affa827134b4598a2feb3c53047c`; build run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/k3_batch3_earlyclobber_build_20260714_124903`.
- Saved the fixed artifact as `k3_v3_batch3_earlyclobber.so` and reran the exact EP16 token512 fault bucket first. It passed graph correctness (`max_abs=0.000488281`), completed all 20 replays without VM fault, and left no KFD PIDs. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep16_k3_batch3_earlyclobber_t512_20260714_125310`. Old pre-fix performance numbers are now invalid for branch selection; remeasure the fixed binary.
- Fixed-binary EP8 full matrix passed all graph precision checks with medians `0.5039/0.6464/0.6776/0.7627/1.0451/1.8451 ms`. Against the recent saved-original matrix this is `+0.20%/+1.53%/+2.61%/+1.99%/+0.92%/-0.43%`; only token64 is clearly beyond the 2% gate and token512 is a sub-noise regression. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep8_k3_batch3_earlyclobber20_20260714_125442`.
- Fixed-binary EP16 full matrix also passed all six precision checks and the formerly failing token512. Medians were `0.3640/0.4104/0.4773/0.5894/1.0002/1.9389 ms`; versus the saved-original EP16 matrix this is `+0.84%/+2.14%/+3.98%/+3.60%/+0.56%/+0.49%`. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep16_k3_batch3_earlyclobber20_20260714_125554`. Proceed to adjacent high-replay EP16 confirmation.
- Completed adjacent EP16 100-replay original/fixed A/B for tokens `32/64/128`. Original medians were `0.41867/0.49750/0.61356 ms`; fixed batch3 medians were `0.41342/0.47908/0.60732 ms`, for `+1.25%/+3.70%/+1.02%`. Both runs passed graph precision and left clean cards. Runs: `ep16_k3_batch3_fixed_ab2_baseline100_mid_20260714_125748` and `ep16_k3_batch3_fixed_ab2_candidate100_mid_20260714_125901`.
- Fixed batch3 EP16 `single-local-rank` token128 passed eager and graph correctness against `ll-masked`; both reported `max_abs=0.000244141`, and no KFD PIDs remained. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_flash_ll_opt/ep16_k3_batch3_fixed_skew_t128_20260714_130049`.
- Began final code-object validation with installed DTK tools: `dccobjdump` is `/root/yuguo/dtk-26.04.1/bin/dccobjdump`, `llvm-readelf` is available under the DTK LLVM tree, and `hipcc` is the DTK compiler. Reconfirmed this `dccobjdump` uses `--extract-elf=all --output=<path>` and `--show-sass`; its help command has the previously recorded nonstandard exit behavior, so inspection wrappers keep it non-fatal.
- `dccobjdump` extraction retries exposed two CLI quirks. The first call passed the `.so` positionally and failed because this build requires `--inputs=<file>`. Retrying with `--inputs` listed the embedded gfx938 ELF correctly, but `--extract-elf=all --output=<directory-or-file>` printed only the host ELF header and did not create the requested output path. No source/runtime files changed; locate any default-output artifact and fall back to compiler save-temps if extraction remains unreliable.
- Located the extracted ELFs in `dccobjdump`'s implicit current-directory outputs despite the ignored requested path, and copied them into the run-local ISA directory. `llvm-readelf` confirms the fixed Flash combine/reduce symbol is `44056` bytes versus original `43224` (`+832`), while Pro is `45112` versus `44248` (`+864`).
- Final resource metadata is unchanged for the Flash combine/reduce kernel: wave64, `39` VGPR, `100` SGPR, `4` SGPR spills, `0` VGPR spills, and `0` private segment for both original and fixed batch3.
- `dccobjdump --show-sass` successfully generated two ~24.8 MB files, but each reported the target symbol at line 1 and identical global-load counts/file size; this reconfirms the tool's concatenated/incomplete internal-body rendering and is not accepted as sequence proof. The DTK tree contains no `llvm-objdump`, only `dccobjdump`; proceed to compiler save-temps/assembly output.
- Recovered the exact K3 V3 HIP compile command from `build/temp/build.ninja` via `ninja -t commands` and saved it in the ISA run directory. It is a 1925-byte `hipcc` command targeting gfx938; the production object path and all include/definition flags are now known, so a run-local `--save-temps=obj` recompile can preserve assembly without modifying the built runtime artifact.
- Completed the isolated production-command recompile with `--save-temps=obj` in `k3_earlyclobber_isa/save_temps_fixed`; it did not overwrite the production object/runtime. The compiler emitted a 20.6 MB gfx938 `.s`, device BC/object/out files, and an isolated host object.
- Compiler assembly provides the missing sequence proof. Four triple-load groups were found: two in Flash (`hidden=4096`) and two in Pro (`hidden=7168`). Each group is exactly three adjacent `global_load_dwordx4` instructions followed by one `s_waitcnt vmcnt(0)`. For example Flash lines 668-671 use output ranges `v[5:8]/v[9:12]/v[14:17]` and address ranges `v[20:21]/v[22:23]/v[24:25]`, proving early-clobber kept all destinations disjoint from the later inputs.
- Ran matching EP8 token64 graph-only `hipprof --stats --hip-trace --follow-fork --devices 0` profiles for original and fixed binaries. Both completed and left clean cards. With only four target-kernel samples, instrumented K3 combine averages were `2.1428 ms` original versus `2.1505 ms` fixed, a sub-1% difference opposite the high-replay end-to-end result; this sample count is too small for attribution. Runs: `ep8_k3_profile_t64_baseline_20260714_130923` and `ep8_k3_profile_t64_fixed_20260714_131028`.
- The fixed profile emitted a transient PyTorch TCPStore heartbeat warning during shutdown, but the harness/hipprof returned status 0, produced the database/statistics, and left no KFD PIDs. Treat it as teardown noise, not a correctness failure. Repeat profiling with 100 graph replays to obtain roughly two orders of magnitude more target-kernel samples.
- Completed matching 100-replay EP8 token64 profiles, each with 110 samples of the target combine kernel. Original K3 combine averaged `170.308 us`; fixed batch3 averaged `156.672 us`, an `8.01%` reduction under instrumentation. The K3 local GEMM stayed close (`171.037 us` original versus `169.555 us` fixed), supporting attribution to the intended combine/reduce wait window. Runs: `ep8_k3_profile_t64_baseline100_20260714_131155` and `ep8_k3_profile_t64_fixed100_20260714_131302`.
- Added a source-contract regression test that requires exactly three ordered `global_load_dwordx4` instructions before the wait and requires `=&v` on all three outputs. This locks both the optimization shape and the early-clobber safety fix.
- Local Windows `python` initially lacked pytest, so installed `pytest 9.1.1` into the configured system Python as required. Local collection then failed before every test body because this workspace has no compiled Windows `megamoe._C`; do not interpret those 14 identical import errors as test failures. The DCU container is the authoritative environment.
- Synced the test file to 151.1 with matching SHA256 `7349f8e71f5a5aedc68548d120a3eda8eedd78af66a1ef2ca3ff0d581201530d`. Final remote source-contract suite passed `14 passed in 7.40s`; runtime SHA remained fixed batch3 `e32d9b94784d584ad7fdd0aaf699938742d4affa827134b4598a2feb3c53047c`, and no KFD PIDs remained.
- Final local review passed: `git diff --check` is clean; intended modifications are the K3 implementation, its source-contract test, the README graph-capture wording, and the three active planning records. All six touched text files remain LF with no UTF-8 BOM. No commit was created.
- Final requested `ll-masked` comparison from the fixed-binary full matrices: EP8 token `8/32/64/128/256/512` gains are `17.56%/12.34%/14.49%/16.15%/9.28%/12.87%`; EP16 gains are `19.10%/16.24%/12.45%/11.93%/13.49%/4.66%`. All associated graph correctness checks passed.
- Started the requested Pro-size validation of the retained K3 batch3/early-clobber branch. The fixed contract uses Pro `384x topk6`, hidden `7168`, intermediate `3072`, EP8/EP16, capacity/token ceiling 512, graph buckets `8,32,64,128,256,512`, and same-run `LL` versus `ll-masked`; representative skew and original/fixed attribution follow the full matrices.
- Pro preflight passed on 151.1: `hy-smi --showpids` reported no KFD PIDs; local/remote K3 header SHA256 both equal `7cadcc4041ece3ea9cd7c4aa2da3dd5aa3c333a48e03d8c1286f68d10eccbb5e`; local/remote source-contract test SHA256 both equal `7349f8e71f5a5aedc68548d120a3eda8eedd78af66a1ef2ca3ff0d581201530d`; source-tree runtime K3 V3 extension SHA256 is the retained fixed artifact `e32d9b94784d584ad7fdd0aaf699938742d4affa827134b4598a2feb3c53047c`.
- Operational note: the first runtime-hash probe used the short `.so` name and failed because the actual extension has the CPython ABI suffix; the exact ABI-suffixed path verified correctly. No file or node state changed.
- First Pro EP8 matrix attempt stopped in `deep_ep.Buffer` initialization because the inherited Flash heap (`4737418240`) was below Pro's reported `11450456192`-byte RDMA allocation. Run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_pro_ll_k3_validation/ep8_fixed_full_20260714_132255`; status 1, no benchmark result, runtime SHA unchanged, and post-run KFD PID state clean.
- Historical Pro records identify the already validated remedy: set both `ROCSHMEM_HEAP_SIZE` and `DUSHMEM_HEAP_SIZE` to `12884901888` (12 GiB). Proceeding with that exact existing configuration rather than changing kernel/test semantics.
- Pro EP8 fixed-K3 full matrix passed with per-bucket capture, graph warmup 3, timing warmup 3/repeat 20, replay soak 20, and same-run `ll-masked`. Fused medians were `1.43582/1.90560/1.97538/2.12872/2.45262/3.95552 ms`; baseline medians were `1.44408/2.08698/2.17298/2.54806/3.19218/5.74220 ms`, yielding `+0.57%/+8.69%/+9.09%/+16.46%/+23.17%/+31.11%`. All six graph precision checks passed; max_abs stayed `<=0.000976562`.
- Pro EP8 run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_pro_ll_k3_validation/ep8_fixed_full_heap12g_20260714_132454`. Status 0, retained runtime SHA unchanged, post-run KFD PID state clean.
- Pro EP16 fixed-K3 full matrix passed under the identical per-bucket graph protocol. Fused medians were `1.09655/1.20865/1.28934/1.47813/2.39638/4.14787 ms`; baseline medians were `1.14419/1.28061/1.40935/1.71316/2.84173/5.10637 ms`, yielding `+4.16%/+5.62%/+8.52%/+13.72%/+15.67%/+18.77%`. All six precision checks passed; max_abs stayed `<=0.000976562`.
- Pro EP16 run: `/root/yuguo/DeepGEMM/hygon_tmp/supernode_debug/151_1_pro_ll_k3_validation/ep16_fixed_full_heap12g_20260714_132641`. Status 0, retained runtime SHA unchanged, post-run KFD PID state clean.
