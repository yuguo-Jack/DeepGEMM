...
def swiglu_quant_channelwise_out(
    ...
    *,
    ...
    actual_m: torch.Tensor | None = None,
    m_per_expert: int = 0,
    ...
    ext.swiglu_quant_channelwise_out(
        ...
        -1 if max_row_blocks is None else int(max_row_blocks),
        actual_m.contiguous() if actual_m is not None else None,
        int(m_per_expert),
...
def swiglu_quant_int8_channelwise_out(
    x: torch.Tensor,
    topk_weights: torch.Tensor | None,
    ...
    *,
    ...
    if x.shape[1] != hidden * 2:
        raise ValueError("x hidden dimension must be even")
    if num_per_channels != hidden:
        raise ValueError("K2 fused INT8 implements channelwise one-scale-per-row quant only")
    if use_col_major_scales:
        raise ValueError("K2 fused INT8 supports use_col_major_scales=False only")
    if round_scale:
        raise ValueError("K2 fused INT8 supports round_scale=False only")
    if ue8m0_scale:
        raise ValueError("K2 fused INT8 supports ue8m0_scale=False only")
    if out_int8.dtype != torch.int8:
        raise ValueError("out_int8 must have torch.int8 dtype")
    ...
    ext.swiglu_quant_int8_channelwise_out(
        x.contiguous(),
        weights.contiguous(),
        out_int8,
        out_scale,
    ...
    out_scale_view = out_scale.view(1, x.shape[0])
    if output_bf16:
        return out_int8, out_scale_view, out_bf16
    return out_int8, out_scale_view
